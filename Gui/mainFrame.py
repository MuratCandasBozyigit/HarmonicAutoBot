...
def build_gui(root):
    ...
    ctk.CTkLabel(control_frame, text="Coin (BTC vs):").grid(row=0, column=0, padx=5)
    ...
    ctk.CTkLabel(control_frame, text="Bar Sayısı:").grid(row=0, column=5, padx=5)
    globals.limit_var = ctk.IntVar(value=100)
    ctk.CTkSegmentedButton(control_frame, values=["50", "100", "250", "500", "1000"],
                           variable=globals.limit_var).grid(row=0, column=6, padx=5)
    ...
    emir_btn.grid(row=0, column=7, padx=5)

    ctk.CTkButton(control_frame, text="Anlık Long Aç", command=Order.execute_trade).grid(row=0, column=8, padx=5)

    ctk.CTkButton(control_frame, text="⚙️ Ayarlar", command=lambda: open_settings_window(root)).grid(row=0, column=9, padx=5)
    ctk.CTkButton(control_frame, text="☼/☾ Tema Değiştir", command=toggle_theme).grid(row=0, column=10, padx=5)

    # 🔄 Oto Yenileme Switch’i
    refresh_switch = ctk.CTkSwitch(control_frame, text="🔄 Oto Yenileme", variable=globals.should_auto_refresh)
    refresh_switch.grid(row=0, column=11, padx=5)
