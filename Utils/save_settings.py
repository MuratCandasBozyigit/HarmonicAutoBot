...
def open_settings_window(root):
    ...
    ctk.CTkLabel(win, text="API Secret:").pack(pady=5)
    api_secret_entry = ctk.CTkEntry(win, width=300, show="*")
    ...
    ctk.CTkLabel(row1, text="Kaldıraç:").grid(row=0, column=0, padx=5)
    leverage_entry = ctk.CTkEntry(row1, width=100)
    leverage_entry.insert(0, str(globals.leverage))
    leverage_entry.grid(row=1, column=0, padx=5)

    ctk.CTkLabel(row1, text="USDT Tutarı:").grid(row=0, column=1, padx=5)
    usdt_entry = ctk.CTkEntry(row1, width=100)
    usdt_entry.insert(0, str(globals.usdt_amount))
    usdt_entry.grid(row=1, column=1, padx=5)

    row2 = ctk.CTkFrame(grid_frame)
    row2.pack(pady=5)

    ctk.CTkLabel(row2, text="Take Profit %:").grid(row=0, column=0, padx=5)
    tp_entry = ctk.CTkEntry(row2, width=100)
    tp_entry.insert(0, str(globals.tp_percent))
    tp_entry.grid(row=1, column=0, padx=5)

    ctk.CTkLabel(row2, text="Stop Loss %:").grid(row=0, column=1, padx=5)
    sl_entry = ctk.CTkEntry(row2, width=100)
    ...
    exchange_var = ctk.StringVar(value=globals.exchange_name.capitalize())
    exchange_option = ctk.CTkOptionMenu(win, variable=exchange_var, values=["Binance", "Bybit", "MEXC", "Bitget", "OKX", "Gate.io", "KuCoin"])
